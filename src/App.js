...
const FORM_FIGURAS =
  "https://docs.google.com/forms/d/e/1FAIpQLSdtWmSw4mD9elpPmY1Owv0jryhlgGZmtueT4_3iSyQ6Ywb8UA/viewform?usp=publish-editor";
const FORM_PALACETES =
  "https://docs.google.com/forms/d/e/1FAIpQLSfhhBzf1xGezrrPKaet0kd8pwnG371yVlxSwCsMu76_Q9E2aQ/viewform?usp=publish-editor";
const FORM_ARTIGOS =
  "https://docs.google.com/forms/d/e/1FAIpQLSfY9tWHignjRqzL30Yig7Zv0snjMYkCX2wU94HGpN2OkJoh6g/viewform?usp=publish-editor";
...
const normalizeKey = (rawKey) => {
  ...
  if (
    ...
    lower.includes("imagens") ||
    lower === "images"
  )
    return "images";
  if (
    lower.includes("palacetes relacionados") ||
    lower.includes("palacetes mencionados") ||
    lower === "relatedpalacetes"
  )
    return "relatedPalacetes";
  if (
    lower.includes("artigos") ||
    lower.includes("temas") ||
    lower === "relatedarticles"
  )
    return "relatedArticles";
  if (lower.includes("bibliografia")) return "bibliografia";
  if (lower.includes("proprietários") || lower === "ownerids")
    return "ownerIds";
  ...
  if (lower.includes("figuras mencionadas") || lower === "relatedfigures")
...
const fetchSheetData = async (sheetName) => {
  ...
  try {
    ...
    let headers = data.table.cols.map((c) =>
    ...
    if (!headers[0] && rows.length > 0) {
      headers = rows[0].c.map((cell) =>
        cell && cell.v ? cell.v.toString().trim() : ""
      );
      rows = rows.slice(1);
    }
        ...
        if (key === "images") {
          obj[key] = value
            ? value
                .toString()
                .split(",")
                .map((s) => getDirectImageUrl(s.trim()))
            : [];
        } else if (
          [
            "ownerIds",
...
export default function App() {
  ...
  // Navegação Centralizada (Limpa os filtros ao mudar de página)
  ...
  const getOwnerForPalacete = (ownerId) =>
    figures.find((f) => f.id === ownerId);
  const getPalacetesForFigure = (houseIds) =>
    palacetes.filter((p) => houseIds?.includes(p.id));
  const getArtigosRelacionados = (id, type) =>
    artigos.filter(
      (art) =>
        (type === "figura" && art.relatedFigures?.includes(id)) ||
        (type === "palacete" && art.relatedPalacetes?.includes(id)) ||
        (type === "artigo" &&
          (art.relatedFigures?.includes(id) ||
            art.relatedPalacetes?.includes(id)))
    );

  // === LÓGICA DO DIRETÓRIO GEOGRÁFICO ===
  // 1. Extrair todos os distritos únicos (convertidos para String pura para evitar crash React)
  const distritosList = [
    ...new Set(
      palacetes.map((p) =>
        p.distrito ? String(p.distrito).trim() : "Não Definido"
      )
    ),
  ]
    .filter(Boolean)
  ...
  // 2. Extrair concelhos baseados no distrito selecionado
  ...
  // 3. Filtrar a grelha de palacetes com base na seleção
  const palacetesFiltrados = palacetes.filter((p) => {
    ...
    if (filterLocation.distrito !== "Todos" && d !== filterLocation.distrito)
      return false;
    if (filterLocation.concelho !== "Todos" && c !== filterLocation.concelho)
      return false;
    return true;
  });
    ...
    <div className="min-h-screen bg-[#faf9f6] text-gray-800 font-sans flex flex-col">
      ...
      <header className="bg-[#11121a] text-white sticky top-0 z-50 shadow-xl border-b border-amber-900/20">
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          >
            <div className="w-10 h-10 bg-amber-600 rounded-sm flex items-center justify-center font-serif text-xl font-bold">
              T
            </div>
            <div className="flex flex-col">
              <span className="font-serif font-bold text-lg uppercase leading-tight tracking-tight">
                Torna-Viagem
              </span>
          ...
          <nav className="flex flex-wrap justify-center items-center gap-3 sm:gap-6 text-[10px] font-bold uppercase tracking-widest">
            ...
            >
              <Database className="w-3 h-3" /> Gestão
            </button>
          </nav>
        </div>
      </header>
      ...
      {view === "gestao" && (
        <main className="flex-1 max-w-4xl mx-auto w-full p-8">
          <div className="bg-white border border-gray-200 rounded-sm shadow-sm overflow-hidden text-center p-6 md:p-12">
            <Database className="w-16 h-16 text-gray-300 mx-auto mb-6" />
            <h2 className="text-3xl font-serif text-[#1a1c29] mb-4">
              Curadoria de Dados
            </h2>
            <p className="text-gray-600 max-w-xl mx-auto mb-10 leading-relaxed text-sm">
              A introdução de novos registos é feita através dos formulários
              oficiais do Google. Ao submeter um formulário, a plataforma será
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 max-w-3xl mx-auto">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex flex-col items-center justify-center gap-3 p-6 bg-[#faf9f6] border border-gray-200 rounded-sm hover:border-amber-500 hover:shadow-md transition-all group"
              >
                ...
                <span className="text-[10px] text-gray-400 flex items-center gap-1">
                  Abrir Formulário <ExternalLink className="w-3 h-3" />
                </span>
              </a>
            ...
            <button
              onClick={loadData}
              className="mt-12 px-4 py-2 border border-gray-200 rounded-sm text-[10px] uppercase tracking-widest text-gray-500 hover:text-amber-600 hover:border-amber-600 flex items-center justify-center gap-2 mx-auto transition-colors"
            >
      ...
      {view === "home" && (
        <main className="flex-1 flex flex-col items-center justify-center p-6 md:p-10 text-center">
          <span className="text-amber-600 font-bold tracking-[0.3em] uppercase text-[10px] mb-4">
            Homenagem aos Torna-Viagem
          </span>
          <h1 className="text-5xl md:text-8xl font-serif text-[#11121a] mb-6 leading-none italic">
            O Legado <span className="not-italic font-bold">Material</span>
          </h1>
          <p className="text-base md:text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed font-light mb-12">
            Exploração prosopográfica e arquitetónica do impacto do capital de
            retorno oitocentista na paisagem portuguesa.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <button
              onClick={() => handleNav("patrimonio")}
              className="bg-[#11121a] text-white px-6 md:px-8 py-3 md:py-4 rounded-sm text-[10px] font-bold uppercase tracking-widest hover:bg-amber-800 transition-colors"
            >
              Explorar Património
            </button>
            <button
              onClick={() => handleNav("figuras")}
              className="border border-gray-300 text-gray-700 px-6 md:px-8 py-3 md:py-4 rounded-sm text-[10px] font-bold uppercase tracking-widest hover:border-amber-600 transition-colors"
            >
              Protagonistas
            </button>
            <button
            ...
            <button
              onClick={() => handleNav("gestao")}
              className="border border-amber-600 text-amber-700 px-6 md:px-8 py-3 md:py-4 rounded-sm text-[10px] font-bold uppercase tracking-widest hover:bg-amber-600 hover:text-white transition-colors flex items-center gap-2"
            >
              <Database className="w-3 h-3" /> Acesso à Gestão
            </button>
          </div>
        </main>
      )}
          ...
          <aside className="w-full lg:w-1/4 lg:flex-shrink-0">
            <div className="bg-white border border-gray-200 rounded-sm p-6 sticky top-28 shadow-sm">
              <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-4">
                ...
                <h3 className="font-serif text-[#1a1c29] text-xl">
              ...
              <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                ...
                <button
                  ...
                  className={`w-full text-left text-sm font-bold uppercase tracking-widest py-2 px-3 rounded-sm transition-colors ${
                    ...
                    <div key={distrito} className="space-y-1">
                      <button
                        onClick={() =>
                          setFilterLocation(
                            isExpanded
                              ? { distrito: "Todos", concelho: "Todos" }
                              : { distrito, concelho: "Todos" }
                          )
                        }
                        className={`w-full flex items-center justify-between text-sm py-2 px-3 rounded-sm transition-colors ${
                          isExpanded
                      ...
                      {isExpanded && (
                        <div className="pl-3 py-1 space-y-1 border-l-2 border-amber-100 ml-4">
                          <button
                            onClick={() =>
                              setFilterLocation({ distrito, concelho: "Todos" })
                            }
                          ...
                          {getConcelhosPorDistrito(distrito).map((concelho) => (
                            <button
                              key={concelho}
                              onClick={() =>
                                setFilterLocation({ distrito, concelho })
                              }
                              className={`block w-full text-left text-xs py-1.5 px-2 rounded-sm transition-colors ${
                                filterLocation.concelho === concelho
                                  ? "font-bold text-amber-600 bg-amber-50/50"
                                  : "text-gray-500 hover:text-amber-600"
                              }`}
                            >
                              {concelho}
          ...
          <div className="w-full lg:w-3/4">
            <div className="flex justify-between items-end mb-8 border-b border-gray-200 pb-4">
              ...
              <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                {palacetesFiltrados.length} Registos
              </span>
            </div>
              ...
              {palacetesFiltrados.length === 0 ? (
                <div className="col-span-full py-12 text-center border-2 border-dashed border-gray-200 rounded-sm">
                  <p className="text-gray-400 italic">
                    Nenhum palacete registado nesta localização.
                  </p>
                  <button
                    onClick={() =>
                      setFilterLocation({
                    ...
                    className="mt-4 text-xs font-bold text-amber-600 uppercase tracking-widest hover:underline"
                  >
                ...
                palacetesFiltrados.map((palacete) => (
                  <div
                    key={palacete.id}
                    onClick={() => openModal(palacete, "palacete")}
                    className="bg-white border border-gray-100 rounded-sm shadow-sm hover:shadow-md cursor-pointer group flex flex-col h-full overflow-hidden"
                  >
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      <span className="text-[9px] font-bold text-amber-600 uppercase tracking-widest mb-2 flex items-center gap-1">
                        ...
                        {palacete.concelho
                          ? `${palacete.concelho}${
                              palacete.distrito ? `, ${palacete.distrito}` : ""
                            }`
                          : "Localização não especificada"}
                      </span>
                      <h3 className="text-xl font-serif text-[#1a1c29] mb-2">
                        {palacete.name || "Sem Título"}
                      </h3>
                      <p className="text-sm text-gray-600 line-clamp-2 mb-4">
                        {palacete.description || "Sem descrição inserida."}
                      </p>
      ...
      {view === "figuras" && (
        <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-12">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <span className="text-amber-600 font-bold tracking-widest uppercase text-[10px] mb-3 block">
              Diretório Biográfico
            </span>
            <h2 className="text-4xl font-serif text-[#1a1c29] mb-4">
              Os Protagonistas
            </h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {figures.length === 0 ? (
              <p className="text-gray-400 italic col-span-full text-center">
                Nenhuma figura registada na base de dados.
              </p>
            ) : (
              figures.map((fig) => (
                <div
                  key={fig.id}
                  onClick={() => openModal(fig, "figura")}
                  className="bg-white border border-gray-100 p-6 text-center group cursor-pointer hover:shadow-lg transition-shadow rounded-sm flex flex-col h-full"
                >
                  ...
                  <h3 className="text-lg font-serif text-[#1a1c29] mb-1 group-hover:text-amber-700 transition-colors leading-tight">
                    {fig.name || "Desconhecido"}
                  </h3>
                  <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-4">
                    {fig.title || fig.category || "Sem categoria"}
                  </p>
                </div>
              ))
            )}
      ...
      {view === "artigos" && (
        <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-12">
          <div className="border-b border-gray-200 pb-8 mb-12">
            ...
            <h2 className="text-4xl font-serif text-[#1a1c29]">
          ...
          <div className="space-y-8">
            {artigos.length === 0 ? (
              <p className="text-gray-400 italic">
                Nenhum artigo registado na base de dados.
              </p>
            ) : (
              artigos.map((artigo) => (
                <article
                  key={artigo.id}
                  onClick={() => openModal(artigo, "artigo")}
                  className="group cursor-pointer bg-white p-6 md:p-8 rounded-sm shadow-sm border border-gray-100 hover:shadow-md transition-all flex flex-col md:flex-row gap-6"
                >
                  ...
                  <div className="md:w-2/3 flex flex-col justify-center">
                    <span className="text-[9px] uppercase tracking-widest text-amber-600 font-bold mb-2 block">
                      {artigo.category || "Ensaio"}
                    </span>
                    <h3 className="text-2xl font-serif text-[#1a1c29] group-hover:text-amber-700 transition-colors mb-4">
                      {artigo.title || "Sem título"}
                    </h3>
                    <p className="text-gray-600 leading-relaxed text-sm border-l-2 border-gray-200 pl-4 italic line-clamp-3">
                      {artigo.excerpt ||
      ...
      <footer className="bg-[#11121a] text-gray-500 py-12 px-6 border-t border-amber-900/10 mt-auto">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-3 opacity-40 grayscale">
            <div className="w-8 h-8 bg-gray-500 rounded-sm flex items-center justify-center text-white font-serif font-bold text-lg">
              T
            </div>
            <div className="flex flex-col text-white">
              <span className="font-serif text-sm font-bold uppercase">
                Torna-Viagem
              </span>
          ...
          <p className="text-[10px] uppercase tracking-[0.2em] text-center md:text-left">
            © 2026 Observatório do Património Digital — CUCP
          </p>
        </div>
      </footer>
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          <div
            className="absolute inset-0 bg-[#1a1c29]/95 backdrop-blur-sm transition-opacity"
            onClick={closeModal}
          ></div>
          ...
          <div className="relative bg-[#faf9f6] w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-sm shadow-2xl flex flex-col md:flex-row">
            <button
              onClick={closeModal}
              className="absolute top-4 right-4 bg-white/80 hover:bg-white text-gray-900 rounded-full p-2 z-10 transition-colors shadow-sm"
            >
              <X className="w-5 h-5" />
            </button>
                    ...
                    <div className="flex gap-2 p-3 bg-gray-900 overflow-x-auto">
                      {selectedItem.images.map((img, idx) => (
                        <button
                          ...
                          className={`w-14 h-14 flex-shrink-0 border-2 rounded-sm overflow-hidden transition-all ${
            ...
            <div className="md:w-3/5 p-8 md:p-10 flex flex-col">
              ...
              <h3 className="text-3xl font-serif text-[#1a1c29] mb-4 leading-tight">
              ...
              <div className="space-y-4 text-gray-600 font-light leading-relaxed text-sm border-b border-gray-200 pb-6 mb-6">
                ...
                {selectedItem.artistic && (
                  <p className="whitespace-pre-wrap">{selectedItem.artistic}</p>
                )}
              ...
              <div className="mt-auto">
                ...
                <div className="space-y-3">
                    ...
                    <>
                      {selectedItem.ownerIds?.map((ownerId) => {
                        ...
                        return (
                          <button
                            key={ownerId}
                            onClick={() => openModal(owner, "figura")}
                            className="flex items-center gap-3 p-3 bg-white border border-gray-200 hover:border-amber-400 rounded-sm w-full text-left transition-colors group"
                          >
                            <div className="w-10 h-10 rounded-full overflow-hidden flex-shrink-0 bg-gray-200">
                              {owner.images && owner.images[0] ? (
                                <img
                                  src={owner.images[0]}
                                  alt=""
                                  className="w-full h-full object-cover"
                                ...
                                <User className="w-5 h-5 m-auto text-gray-400 mt-2" />
                              )}
                            </div>
                            <div>
                              <p className="text-[9px] uppercase tracking-widest text-amber-600 font-bold mb-0.5">
                                Proprietário
                              </p>
                              <p className="text-sm font-serif text-[#1a1c29] group-hover:text-amber-700">
                                {owner.name}
                              </p>
                            </div>
                  ...
                  {modalType === "figura" && (
                    <>
                      {getPalacetesForFigure(selectedItem.relatedPalacetes).map(
                        (house) => (
                          <button
                            key={house.id}
                            onClick={() => openModal(house, "palacete")}
                            className="flex items-center gap-3 p-3 bg-white border border-gray-200 hover:border-amber-400 rounded-sm w-full text-left transition-colors group"
                          >
                            <div className="w-10 h-10 rounded-sm overflow-hidden flex-shrink-0 bg-gray-200">
                              {house.images && house.images[0] ? (
                                ...
                                <Building className="w-5 h-5 m-auto text-gray-400 mt-2" />
                              )}
                            </div>
                            <div>
                              <p className="text-[9px] uppercase tracking-widest text-amber-600 font-bold mb-0.5">
                  ...
                  {getArtigosRelacionados(selectedItem.id, modalType).map(
                    (art) => (
                      <button
                        key={art.id}
                        onClick={() => openModal(art, "artigo")}
                        className="flex items-center gap-2 bg-slate-50 border border-slate-200 px-3 py-3 rounded-sm text-xs text-slate-700 hover:bg-slate-100 transition-colors w-full text-left group"
                      >
                        <BookOpen className="w-4 h-4 text-amber-600 flex-shrink-0" />
                        <span className="font-bold">Tema / Ensaio:</span>{" "}
                        {art.title}
                        <ChevronRight className="w-4 h-4 ml-auto text-gray-400 group-hover:text-amber-500" />
                      </button>
                    )
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedItem.relatedFigures?.map((id) => {
                        ...
                        return (
                          <button
                            key={id}
                            onClick={() => openModal(fig, "figura")}
                            className="bg-amber-50 text-amber-900 border border-amber-200 px-3 py-2 rounded-sm text-xs hover:bg-amber-100 transition-colors flex items-center gap-2"
                          >
                            <User className="w-3 h-3 text-amber-600" />{" "}
                            <span className="font-bold">{fig.name}</span>
                          </button>
                        );
                      })}
                      {selectedItem.relatedPalacetes?.map((id) => {
                        ...
                        return (
                          <button
                            key={id}
                            onClick={() => openModal(pal, "palacete")}
                            className="bg-slate-100 text-slate-800 border border-slate-200 px-3 py-2 rounded-sm text-xs hover:bg-slate-200 transition-colors flex items-center gap-2"
                          >
